...
- TypeScript 5.x, browser target ES2020+ with current `tsconfig` target ES2022 + Existing Vite app, `dockview-core` panel shell, `sql.js` project database session, no new runtime dependency planned (009-playback-engine-data)
- In-memory `ProjectDatabase` from `DbSessionRef`; local `data/pool` folder tree written through browser File System Access APIs (009-playback-engine-data)
...
## Recent Changes
- 009-playback-engine-data: Added TypeScript 5.x, browser target ES2020+ with current `tsconfig` target ES2022 + Existing Vite app, `dockview-core` panel shell, `sql.js` project database session, no new runtime dependency planned
