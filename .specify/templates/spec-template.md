...
**Browser Surface**: [Which browser-facing page or UI area is affected]
...
- What happens when the local WebSocket engine is unavailable, slow, or returns
  unexpected messages?
- What happens when the browser lacks support for a chosen API despite being in
  the modern support target?
...
- **SC-005**: [Manual visual validation outcome, e.g., "Primary interaction works
  correctly in browser with local engine connected"]
- **SC-006**: [Quality gate outcome, e.g., "Lint/compile checks pass with no new
  errors"]
